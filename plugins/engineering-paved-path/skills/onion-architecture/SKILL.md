...
description: "Enforces Onion Architecture (Ports & Adapters / Clean Architecture) for your backend service's modules. Use when adding a new feature module, refactoring a service, designing a repository, wiring a new adapter, or deciding where a piece of logic belongs. Trigger terms: onion, clean architecture, hexagonal, ports, adapters, dependency rule, domain layer, application layer, repository pattern, DI container, composition root."
allowed-tools: Read, Write, Edit, Bash, Grep, Glob
---

# Onion Architecture — Backend Services

Enforces clean separation of concerns across this codebase's backend package (whatever it's named — e.g. `server/`, `api/`, `backend/`).
...
- [ ] No `new ConcreteAdapter()` inside a service — only inside the composition root (`container.ts`, `app.ts`, or wherever this codebase wires implementations)
