...
Adapters for external APIs live in `adapters/` and implement a port from your shared/domain-contracts
package (if one exists), or from a local `ports.ts`/`types.ts` file if the codebase doesn't have one:

```ts
// adapters/github/octokit.ts
import { Octokit } from '@octokit/rest';
import type { GitHubClient } from '../shared/contracts';
