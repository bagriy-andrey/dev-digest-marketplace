...
**Allowed imports:** TypeScript stdlib, your shared/domain-contracts package's Zod contracts (if one exists), other domain-layer files  
**Forbidden imports:** `fastify`, `drizzle-orm`, `db/schema`, `db/client`, `adapters/`, the composition root (`container.ts`/`app.ts` or equivalent)
...
**Allowed imports:** domain layer, port interfaces from your shared/domain-contracts package (if one exists) or local `types.ts`/`ports.ts` files  
...
**Must implement:** interfaces defined in application or your shared/domain-contracts package (if one exists)
...
**Lives in:** your composition root exclusively (commonly `container.ts`, `app.ts`, or similar — wherever this codebase wires concrete implementations together)  
...
|------|----------------|--------------------|
| `helpers.ts` (domain) | shared/domain-contracts package (if any), other helpers | `db/*`, `adapters/*`, `platform/*`, `fastify` |
| `service.ts` (application) | domain, port interfaces, the composition root (for DI) | `db/schema`, `drizzle-orm`, `fastify`, concrete adapters |
