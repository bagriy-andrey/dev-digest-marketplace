...
Every feature in `<backend-package>/src/modules/<name>/` follows the same canonical layout
(substitute your actual backend package name for `<backend-package>`, e.g. `server/`, `api/`, `backend/`).
