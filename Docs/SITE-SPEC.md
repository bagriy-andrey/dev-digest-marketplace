...
- **Data source**: `browser/src/data.ts` fetches `../index.json` at runtime (`loadIndex()`), which `scripts/build-index.mjs` generates from the real `.claude-plugin/marketplace.json` + each plugin's `plugin.json`/README.md/`skills/*/SKILL.md`/`agents/*.md` — no hardcoded sample data. Local dev syncs a copy to `browser/public/index.json` via `npm run predev` (gitignored, regenerated on every `dev`/`build`).
